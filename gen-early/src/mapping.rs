...
pub(crate) struct Mapping {
    ...
    /// Entities generated as model + serialize only (bind skipped). Mirror of
    /// `read_only` for write-only forms: the entity is emitted (e.g. a complex
    /// carrier normalized on write to a plain simple entity), but its standalone
    /// form is never read (the read goes through another handler), so the
    /// generated `bind` would be dead. Precondition: any enum/select it
    /// serializes must have its emit helper generated by a serialized entity.
    #[serde(default)]
    pub(crate) write_only: Vec<String>,
