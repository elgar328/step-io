...
fn main() {
    ...
    for ent_name in &targets {
        ...
        .unwrap();
        writeln!(model, "#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]").unwrap();
        writeln!(model, "pub(crate) struct {type_name}Id(pub u32);").unwrap();
        writeln!(model).unwrap();
        writeln!(model, "impl crate::ir::arena::ArenaId for {type_name}Id {{").unwrap();
        writeln!(model, "    fn index(&self) -> usize {{").unwrap();
        writeln!(model, "        self.0 as usize").unwrap();
        writeln!(model, "    }}").unwrap();
        writeln!(model, "    fn from_index(index: u32) -> Self {{").unwrap();
        writeln!(model, "        Self(index)").unwrap();
        writeln!(model, "    }}").unwrap();
        writeln!(model, "}}\n").unwrap();
