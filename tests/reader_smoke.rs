...
fn nist_property_def_property_definition_arena() {
    // Schema-faithful `property_definitions` arena (Phase property-definition):
    // reader pushes both `Itself` (plain PD) and `ProductDefinitionShape`
    // variants. The NIST fixture carries many of each; assert the arena is
    // non-empty and contains both variant kinds.
