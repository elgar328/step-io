//! `read()` surfaces the identified source schema on the model's header —
//! `model.header().schema` — how a caller (e.g. a CAD kernel) learns the
//! file's precise AP / edition / stage. Inline STEP docs, not valid-CAD
//! fixtures.
...
fn read_reports_ap242_ed2() {
    ...
    let schema = &model.header().schema;
    assert_eq!(schema.family, ApFamily::Ap242);
    assert_eq!(schema.edition, Some(2));
    assert_eq!(schema.stage, Stage::Is);
    assert_eq!(schema.to_string(), "AP242 ed2 (IS)");
...
fn read_reports_ap214_ed3() {
    ...
    let (model, _report) = read(src.as_bytes()).expect("read ok");
    let schema = &model.header().schema;
    assert_eq!(schema.family, ApFamily::Ap214);
    assert_eq!(schema.edition, Some(3));
    assert_eq!(schema.stage, Stage::Is);
}
...
fn read_reports_unrecognized_schema() {
    ...
    let (model, _report) = read(src.as_bytes()).expect("read ok");
    let schema = &model.header().schema;
    assert_eq!(schema.family, ApFamily::Other);
    assert!(!schema.is_recognized());
}
