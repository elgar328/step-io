...
impl SimpleEntityHandler for RoleAssociationHandler {
    ...
    fn read(
        ...
        // Members generated by `StepSelect`; an unsupported variant
        // (Approval/DTA/etc.) resolves to `None` and drops the entity.
        let Some(item_with_role) = RoleSelect::resolve_select(ctx, item_ref) else {
            return Ok(());
        };
