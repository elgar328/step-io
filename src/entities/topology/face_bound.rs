//! `FACE_BOUND` handler — face boundary wire (2-layer path). The bound loop's
...
impl SimpleEntityHandler for FaceBoundHandler {
    ...
    fn read(
        ...
        let early = bind::bind_face_bound(entity_id, attrs)?;
        lower::lower_face_bound(ctx, entity_id, &early)
    }
    ...
    fn write(
        ...
        let early = lift::lift_face_bound(bound, orientation);
        Ok(serialize::serialize_face_bound(buf, &early))
    }
}
