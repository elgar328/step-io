...
pub(crate) enum PassLevel {
    ...
    Pass7SymbolStyle,
    /// `POINT_STYLE` (phase point-style). Depends on `Pass7Colour`
    /// (`viz_colour_id_map`) + `viz_pre_defined_marker_id_map` + `mwu_id_map`.
    Pass7PointStyle,
    ...
    /// `TEXT_STYLE_WITH_BOX_CHARACTERISTICS` (phase text-style-box). Depends
    ...
    Pass8Gisu,
    /// `INVISIBILITY` (phase invisibility). Depends on
    ...
    Pass8PrCore,
    /// `AREA_IN_SET` + `PRESENTATION_SIZE` (phase pr-size). Depends on
    ...
    /// `APPLIED_APPROVAL_ASSIGNMENT` / `CC_DESIGN_APPROVAL` (Pass 9-11) —
