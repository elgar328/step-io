//! `bind`: `EntityGraph` (late-bound) → `EarlyModel` (L1). Mechanical attribute
//! extraction; no SELECT resolution / flatten / arena lookups (those are
//! [`lower`](super::lower)'s job). All bind fns are now generated by `gen-early`
//! (`super::generated::bind`) and re-exported here so the handlers keep their
//! `bind::bind_*` paths unchanged.

pub(crate) use super::generated::bind::{
    bind_address, bind_application_context, bind_application_protocol_definition,
    bind_applied_approval_assignment, bind_applied_date_and_time_assignment,
    bind_applied_document_reference, bind_applied_external_identification_assignment,
    bind_applied_group_assignment, bind_applied_person_and_organization_assignment,
    bind_applied_security_classification_assignment, bind_approval, bind_approval_date_time,
    bind_approval_person_organization, bind_approval_role, bind_approval_status,
    bind_calendar_date, bind_cc_design_approval, bind_cc_design_date_and_time_assignment,
    bind_cc_design_person_and_organization_assignment, bind_cc_design_security_classification,
    bind_colour_rgb, bind_constructive_geometry_representation_relationship,
    bind_context_dependent_shape_representation, bind_coordinated_universal_time_offset,
    bind_date_and_time, bind_date_time_role, bind_derived_unit, bind_derived_unit_element,
    bind_description_attribute, bind_dimensional_exponents, bind_document, bind_document_file,
    bind_document_product_equivalence, bind_document_representation_type, bind_document_type,
    bind_draughting_pre_defined_colour, bind_draughting_pre_defined_curve_font,
    bind_fill_area_style, bind_general_property, bind_group, bind_id_attribute,
    bind_identification_role, bind_local_time, bind_mechanical_design_and_draughting_relationship,
    bind_name_attribute, bind_next_assembly_usage_occurrence, bind_object_role, bind_organization,
    bind_person, bind_person_and_organization, bind_person_and_organization_role,
    bind_personal_address, bind_point_style, bind_pre_defined_curve_font, bind_pre_defined_marker,
    bind_pre_defined_point_marker_symbol, bind_pre_defined_symbol,
    bind_pre_defined_terminator_symbol, bind_product, bind_product_definition,
    bind_product_definition_formation, bind_product_definition_formation_with_specified_source,
    bind_product_definition_shape, bind_product_definition_with_associated_documents,
    bind_representation_relationship, bind_role_association, bind_security_classification,
    bind_security_classification_level, bind_shape_representation_relationship,
    bind_surface_side_style, bind_surface_style_fill_area, bind_surface_style_usage,
    bind_symbol_colour, bind_view_volume,
};
