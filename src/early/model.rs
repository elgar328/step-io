...
/// members the bespoke `viz_*_id_map` fields could not — e.g.
/// `surface_side_style` probes `id_cache.get::<EarlySurfaceStyleFillAreaId>(r)`
/// where the seven maps' shared value type (`FoundedItemId`) would have
/// collided in a single bucket. The distinct `Early*Id` key types are generated
/// per entity by `gen-early` (in `generated/model.rs`).
#[derive(Default)]
